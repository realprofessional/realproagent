...
if ($tasksLists) {
    foreach ($tasksLists as $tasksList) {
        ?>
        <li class="task" id="task_li_section{{ $tasksList->id }}" data-index="0" data-task-index="{{ $tasksList->id }}" data-board-id="{{ $board->id }}" data-task-position="{{ $tasksList->task_position }}" >
                ...
                <div class="task-actions">
                    ...
                    <div class="copy_taskk_show " id="copy_taskk_show{{ $tasksList->id }}" style="display:none">
                        <div class="copy_heading_task">
                            <div class="copy_heading_task_one">Copy Task</div>
...
<a href="{{ HTTP_PATH.'board/'.$project->slug.'/'. $board->slug .'/'.$tasksList->slug }}" >
                        <p class="<?php echo $tasksList->is_checked == 1 ? 'crossed_ttd' : ''; ?>" id="test_name_data{{ $tasksList->id }}">{{ $tasksList->task_name }}</p>
                    </a>

                    <span class="due_on_txt" id="due_on_txt_{{ $tasksList->id }}">
                        <?php if(!empty($tasksList->due_date)){
                ...
                <?php if ($i == 0 ) { 
                    if(!empty($slug3)){
                        ...
                        $(document).ready(function(){
                            $(function () {
                                fetchTaskDetails1('<?php echo $slug3 ?>');
                            });
                        });
                    </script>
                        <?php
                ...
                <script>
                        $(document).ready(function(){
                            $(function () {
                                fetchTaskDetails1('<?php echo $tasksList->slug ?>');
                            });
                        });
                    </script>
                        <?php
