...
<div class="loader" id="loader_full" style="display:none;">
    {{ Html::image('public/img/front/loader.gif','img',array('class'=>"")) }}
</div>
...
<div class="acc_deack_new_ sdfwe">
    <div class="wrapper_new">
        <?php /* ?> <div class="df_ty">
          ...
          <div class="dd_showmenu">
          <!--                <a href="javascript:void(0)" class="showhidemenu">Show Menu</a>-->
...
<script>
    $(document).ready(function(){
        $(function () {
            updateAllActivity(<?php echo $project->id ?>,<?php echo $bdid; ?>);  
            //setInterval(getBoardContent, 10000);
        });
    });
   
    
    function updateAllActivity(id, id2) { 
...
<script type="text/javascript">
    ...
    function fetchTaskDetails(id) {
        if (id == '') {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo HTTP_PATH; ?>board/displayProjectTaskSection/'+id,
                //data: $('#addtask' + id).serialize(),
                beforeSend: function () {
                    $('html, body').css("cursor", "wait");
                    $("#loader_full").show();
                ...
                success: function (result) {
                    $('html, body').css("cursor", "auto");
                    $("#loader_full").hide();
                    $(".insecption").html(result);
                    
                    //                    alert(typeof(CKEDITOR) + 'asdasda'); 
        ...
        if (id == '') {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    //                    var editor = CKEDITOR.instances.email_content;
                    //                    if (editor) {
    ...
    function submitTaskChecboxValueData(id) {
    
        $.ajax({
            type: 'POST',
            url: '<?php echo HTTP_PATH; ?>board/updateTaskCheckBox',
            data: $('#checkboxTaskTick' + id).serialize(),
            beforeSend: function () {
                $('html, body').css("cursor", "wait");
            },
            success: function (result) {
                ...
                if($("#test_name_data"+id).hasClass('crossed_ttd')){
                    $("#test_name_data"+id).removeClass('crossed_ttd');
                }else{
                    $("#test_name_data"+id).addClass('crossed_ttd');
                }
